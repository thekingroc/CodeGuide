...
那你说，既然 if...else 实现的这么快，还考虑数据结构、算法逻辑、设计模式、系统架构吗？当然这基本要看你的项目在可预见下能活多久，如果一个项目至少存活一年，并且在这一年中又会不断的的迭代。比如你做了一个营销优惠券系统，在各种条件下发放各种类型的券。如果在最开始没有考虑好系统设计和架构模式，那么当`活动频发`、`流量暴增`、`需求迭代`下、最后你可能会挂在系统事故上！
